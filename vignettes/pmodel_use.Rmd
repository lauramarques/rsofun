...
params_modl <- list(
    ...
    added_calibratable_parameter = XXX jaideep XXX
  )
...
model_data <- output %>%
  ...
  tidyr::unnest(data) %>% # Unnests the list with one dataframe
  tidyr::unnest(data) # Now it unnests the dataframe
